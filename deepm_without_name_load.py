...
def main():
    ...
    if curr_date == date_validator(report_id_base, "25") and curr_date == date_validator(report_id_7d, "25") and curr_date == date_validator(report_id_3d, "25") and curr_date == date_validator(report_id_1d, "25") and curr_date == date_validator(report_id_1h, "25") and not file.exists():
        timestamp = datetime.now()
        print("Data loading to bigquery Started at ", timestamp)
        subprocess.call("./deepm_without_name_transfer.sh " + advertiser_name + "  " + report_id_base + " " + report_id_7d + " " + report_id_3d + " " + report_id_1d + " " + report_id_1h, shell = True)
        timestamp = datetime.now()
