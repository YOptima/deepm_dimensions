...
sudo bq load --source_format CSV --skip_leading_rows 1 --allow_jagged_rows --replace _deepm.$1 $2.csv schema_30D.json
sudo rm -rf $2*

sudo python3 deepm_transformation.py $3
sudo bq load --source_format CSV --skip_leading_rows 1 --allow_jagged_rows --replace _deepm.$1_7D $3.csv schema_7D.json
sudo rm -rf $3*

sudo python3 deepm_transformation.py $4
sudo bq load --source_format CSV --skip_leading_rows 1 --allow_jagged_rows --replace _deepm.$1_3D $4.csv schema_3D.json
sudo rm -rf $4*

sudo python3 deepm_transformation.py $5
sudo bq load --source_format CSV --skip_leading_rows 1 --allow_jagged_rows --replace _deepm.$1_1D $5.csv schema_1D.json
sudo rm -rf $5*

sudo python3 deepm_transformation.py $6
sudo bq load --source_format CSV --skip_leading_rows 1 --allow_jagged_rows --replace _deepm.$1_1H $6.csv schema_1H.json
sudo rm -rf $6*
...
sudo bq query --use_legacy_sql=false 'UPDATE `_deepm.'$1'` a
    ...
    IFNULL(a. Insertion_Order ,"") = IFNULL(b. Insertion_Order ,"") AND
    ...
    IFNULL(a. Campaign ,"") = IFNULL(b. Campaign ,"") AND
    IFNULL(a. Floodlight_Activity_ID ,0) = IFNULL(b. Floodlight_Activity_ID ,0) AND
    IFNULL(a. Floodlight_Activity ,"") = IFNULL(b. Floodlight_Activity ,"") AND
    IFNULL(a. Creative_ID ,0) = IFNULL(b. Creative_ID ,0) AND
    IFNULL(a. Creative ,"") = IFNULL(b. Creative ,"") AND
    ...
    IFNULL(a. CM_Placement_ID ,0) = IFNULL(b. CM_Placement_ID ,0) AND
    IFNULL(a. Device_Type ,"") = IFNULL(b. Device_Type ,"") AND
    IFNULL(a. Environment ,"") = IFNULL(b. Environment ,"")'
sleep 5
...
sudo python3 lookback_historical.py $1
